...
export default async function wrapInScripts (markup, { title, now, input_file_name, raw, meta, livereload }) {
  ...
  let styles = await fs.readFile('./lib-browser/styles.css');
  const license = await fs.readFile('./LICENSE');
...
async function loadCSVParse () {
  const license = (await fs.readFile('./node_modules/csv-parse/LICENSE')).toString();
...
async function loadPrism () {
  const license = (await fs.readFile('./node_modules/prism-code-editor/LICENSE')).toString();
  const styles = [
    (await fs.readFile('./node_modules/prism-code-editor/dist/layout.css')).toString(),
    (await fs.readFile('./node_modules/prism-code-editor/dist/scrollbar.css')).toString(),
    (await fs.readFile('./node_modules/prism-code-editor/dist/themes/prism.css')).toString(),
  ].join('');
  function injectLicense (script_content) {
...
async function loadChartist () {
  const license = (await fs.readFile('./node_modules/chartist/LICENSE-MIT')).toString();
  const styles = (await fs.readFile('./node_modules/chartist/dist/index.css')).toString().replace('/*# sourceMappingURL=index.css.map */','');
